...
function SectionHeader({ icon: Icon, title }) {
  return (
    <div className="flex items-center gap-2.5 mb-4">
      <div className="w-9 h-9 rounded-xl bg-brand-gradient-soft flex items-center justify-center">
    ...
    <div className="grid lg:grid-cols-[1.1fr,0.9fr] gap-5">
      ...
      <div className="space-y-5">
        ...
        <GlassCard className="p-6">
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            ...
            <Input label="Location" placeholder="Austin, TX" className="sm:col-span-2" value={data.personal.location} onChange={(e) => updatePersonal("location", e.target.value)} />
          </div>
        </GlassCard>
        ...
        <GlassCard className="p-6">
          ...
          <div className="flex flex-wrap gap-2 mt-3">
            ...
            <Button variant="outline" size="sm" icon={Sparkles} onClick={generateSummary} disabled={improving}>
              Generate Summary ✨
            </Button>
          </div>
        </GlassCard>
            ...
            {data.education.map((e) => (
              <div key={e.id} className="glass-soft rounded-2xl p-4">
                ...
                {data.education.length > 1 && (
                  <button onClick={() => removeListItem("education", e.id)} className="text-xs text-rose-500 mt-2 flex items-center gap-1">
        ...
        <GlassCard className="p-6">
          ...
          <div className="space-y-4">
            {data.experience.map((e) => (
              <div key={e.id} className="glass-soft rounded-2xl p-4 space-y-3">
                <div className="grid sm:grid-cols-3 gap-3">
                ...
                <textarea
                  ...
                  rows={2}
                  className="glass-input w-full rounded-xl p-3 text-sm outline-none focus:ring-2 focus:ring-primary-400/60 resize-none"
                />
                {data.experience.length > 1 && (
                  <button onClick={() => removeListItem("experience", e.id)} className="text-xs text-rose-500 flex items-center gap-1">
                    <Trash2 size={12} /> Remove
                  </button>
                )}
              </div>
        ...
        <GlassCard className="p-6">
          ...
          <div className="space-y-4">
            {data.projects.map((p) => (
              <div key={p.id} className="glass-soft rounded-2xl p-4 space-y-3">
                ...
                {data.projects.length > 1 && (
                  <button onClick={() => removeListItem("projects", p.id)} className="text-xs text-rose-500 flex items-center gap-1">
                    <Trash2 size={12} /> Remove
                  </button>
                )}
              </div>
        ...
        <GlassCard className="p-6">
          ...
          <Input placeholder="React, Node.js, SQL, Docker..." value={data.skills} onChange={(e) => updateSkills(e.target.value)} />
        </GlassCard>
      </div>
      ...
      <div className="lg:sticky lg:top-4 h-fit space-y-4">
        <GlassCard className="p-5">
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-heading font-bold text-ink text-sm">Live Preview</h3>
            <Button variant="primary" size="sm" icon={Download} onClick={exportPDF} disabled={exporting}>
              {exporting ? "Exporting..." : "Download PDF"}
            </Button>
          </div>
          <div className="max-h-[700px] overflow-y-auto rounded-2xl">
