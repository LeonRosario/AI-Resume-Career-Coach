  ...
  return (
    <div className="space-y-6">
      <GlassCard className="p-5 flex flex-wrap items-center gap-3">
        <div className="flex items-center gap-2 text-muted text-sm font-medium mr-1">
        ...
        {filtered.map((job, i) => (
          <GlassCard key={job.id} hover delay={i * 0.06} className="p-6 flex flex-col">
            ...
            <div className="flex items-center justify-between mt-auto pt-3 border-t border-[rgba(0,132,255,0.1)]">
              <span className="text-sm font-semibold text-body">{job.salary}</span>
      ...
      {filtered.length === 0 && (
        <GlassCard className="p-12 text-center">
          <p className="text-muted">No roles match these filters yet. Try widening your search.</p>
        </GlassCard>
      )}
    </div>
  );
